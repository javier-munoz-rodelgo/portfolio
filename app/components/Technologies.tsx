...
export default function Technologies({ dict }: { dict: TechnologiesDict }) {
  ...
  return (
    <section
      id="technologies"
      className="min-h-screen bg-slate-50 dark:bg-slate-900 py-20 sm:py-24"
    ...
    >
      <div className="mx-auto w-full max-w-6xl px-6">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5 dark:border-slate-700 dark:bg-slate-800 dark:shadow-black/20 p-4 sm:p-6">
          <div
          ...
          >
            {activeArea.technologies.map((item, index) => (
              <article
                ...
                className={`group rounded-2xl border border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-white dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-500 dark:hover:bg-slate-800 px-4 py-5 transition-all duration-300 hover:-translate-y-0.5 ${
                  visible
                    ? "translate-y-0 opacity-100"
                    : "translate-y-1 opacity-0"
                }`}
              >
                <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-white ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
                  <Image
                    src={item.logo}
                    alt={item.text}
                    width={56}
                ...
                <p className="text-center text-sm font-medium text-slate-700 group-hover:text-slate-900 dark:text-slate-300 dark:group-hover:text-slate-100">
                  {item.text}
                </p>
              </article>
            ))}
